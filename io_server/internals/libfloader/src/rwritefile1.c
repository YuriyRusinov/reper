...
//        elog(NOTICE, "In NULL LOOP");
    ...
    elog(NOTICE, url);
 */
    url = (char *)palloc(strlen("C:\\Program Files\\PostgreSQL\\9.2\\data\\io_files\\1.txt")+1);
    strcpy(url, "C:\\Program Files\\PostgreSQL\\9.2\\data\\io_files\\1.txt");


    if(mode == 0){
        errno_t err = fopen_s(&fFile, url, "w+b");
    }
    else if(mode == 1){
        errno_t err = fopen_s(&fFile, url, "a+b");
    }
    else if(mode == 2){
        fFile = fopen(url, "rb");
        if(fFile != NULL){
    ...
    if (fFile == NULL){
        //--SPI_finish();
        elog(ERROR, "FOPEN: %s", strerror(errno));
    }
