create or replace function svgPlanGenerator (int4, float8, float8, int4, int4) returns xml as
$BODY$
declare
    id_graph_plan alias for $1;
...
begin
    ...
    select name into svg_title from graph_plans gp where gp.id=id_graph_plan;
    svg_str := svg_str || E'\n' || '    <title>' || svg_title || '</title>';
    for rdev in
        select d.id as id_device, es.id as id_element_shape, es.is_closed from
            devices d inner join
            element_shapes es on (d.id=es.id_device and d.id_graph_plan=id_graph_plan) --inner join
...
create or replace function svgPlanGenerator (int4) returns xml as
$BODY$
declare
    id_graph_plan alias for $1;
begin

    return svgPlanGenerator (id_graph_plan, null::int4, null::int4, 800, 600);
end
$BODY$
language 'plpgsql';

create or replace function svgPlanGenerator1 (int4) returns xml as
$BODY$
declare
    idDevice alias for $1;
    idGraphPlan int4;
begin

    select id_graph_plan into idGraphPlan from devices where id = idDevice;
    if(idGraphPlan isnull) then
        return NULL;
    end if;

    return svgPlanGenerator (idGraphPlan, null::int4, null::int4, 800, 600);
end
$BODY$
language 'plpgsql';
