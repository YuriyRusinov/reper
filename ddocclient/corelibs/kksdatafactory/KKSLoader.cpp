...
/*    else if (QString::compare (tableName, QString("type_ship"), Qt::CaseInsensitive) == 0)
    {
        ...
        attrsWith1 = tableName + ".id, " + 
                     tableName + ".unique_id, " + 
                     tableName + ".last_update "; //колонки в подзапросе нерекурсивной части предложения WITH
    }
    else{
    ...
    for (pca=tc->attributes().constBegin(); pca != tc->attributes().constEnd(); pca++)
    {
        ...
        if(a->code() == "id_io_state"){
            if(tableName.toLower() != "io_objects" && tableName.toLower() != "tbl_io_objects")//в справочнике ИО есть атрибут id_io_state
            continue;
        }
        if(a->code() == "uuid_t"){
...
//загрузка рубрики из записи справочника (с рекурсией, если явно указано)
...
//получаем записи, входящие в рубрику в БД, а также записи, добавленные в рубрику пока локально, исключая удаленные локально
