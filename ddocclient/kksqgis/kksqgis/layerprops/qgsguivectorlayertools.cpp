...
bool QgsGuiVectorLayerTools::addFeature( QgsVectorLayer* layer, QgsAttributeMap defaultValues, const QgsGeometry& defaultGeometry )
{
  ...
  QgsFeatureAction a( mWorkingWidget, tr( "Add feature" ), f, layer );
...
bool QgsGuiVectorLayerTools::startEditing( QgsVectorLayer* layer )
{
  ...
  if ( !layer->isEditable() && !layer->isReadOnly() )
  {
    ...
    {
      mWorkingWidget->messageBar()->pushMessage( tr( "Start editing failed" ),
          tr( "Provider cannot be opened for editing" ),
          QgsMessageBar::INFO, mWorkingWidget->messageTimeout() );
      return false;
    }
  ...
  if ( layer->isModified() )
  {
                                       ...
                                       buttons ) )
    {
      ...
      case QMessageBox::Discard:
        mWorkingWidget->mapCanvas()->freeze( true );
        if ( !layer->rollBack() )
        {
          mWorkingWidget->messageBar()->pushMessage( tr( "Error" ),
              tr( "Problems during roll back" ),
              QgsMessageBar::CRITICAL );
          res = false;
        }
        mWorkingWidget->mapCanvas()->freeze( false );
...
void QgsGuiVectorLayerTools::commitError( QgsVectorLayer* vlayer )
{
  ...
  QgsMessageBarItem *errorMsg = new QgsMessageBarItem(
    ...
    0,
    mWorkingWidget->messageBar() );
  mWorkingWidget->messageBar()->pushItem( errorMsg );

}

void QgsGuiVectorLayerTools::setWorkingWidget( KKSGISWidgetBase * w)
{
    mWorkingWidget = w;
}
