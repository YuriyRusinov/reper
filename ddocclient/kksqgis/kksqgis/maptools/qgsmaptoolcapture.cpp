...
QgsMapToolCapture::QgsMapToolCapture( KKSGISWidgetBase * w, QgsMapCanvas* canvas, enum CaptureMode tool )
    : QgsMapToolEdit( w, canvas )
    , mCaptureMode( tool )
    , mRubberBand( 0 )
    , mTempRubberBand( 0 )
    , mValidator( 0 )
{
  ...
  connect( mWorkingWidget->legend(), SIGNAL( currentLayerChanged( QgsMapLayer * ) ),
...
void QgsMapToolCapture::stopCapturing()
{
  ...
  mWorkingWidget->skipNextContextMenuEvent();
#endif
...
void QgsMapToolCapture::validateGeometry()
{
  ...
  QStatusBar *sb = mWorkingWidget->statusBar();
