...
QgsMapToolNodeTool::QgsMapToolNodeTool( KKSGISWidgetBase * w, QgsMapCanvas* canvas )
    : QgsMapToolVertexEdit( w, canvas )
    , mSelectedFeature( 0 )
    , mMoving( true )
    , mClicked( false )
    , mCtrl( false )
...
void QgsMapToolNodeTool::canvasPressEvent( QMouseEvent * e )
{
  ...
  if ( !mSelectedFeature )
  {
    ...
    mSelectedFeature = new QgsSelectedFeature( mWorkingWidget, snapResults[0].snappedAtGeometry, vlayer, mCanvas );
    connect( mWorkingWidget->legend(), SIGNAL( currentLayerChanged( QgsMapLayer* ) ), this, SLOT( currentLayerChanged( QgsMapLayer* ) ) );
...
void QgsMapToolNodeTool::cleanTool( bool deleteSelectedFeature )
{
  ...
  if ( mSelectedFeature )
  {
    ...
    disconnect( mWorkingWidget->legend(), SIGNAL( currentLayerChanged( QgsMapLayer* ) ), this, SLOT( currentLayerChanged( QgsMapLayer* ) ) );
