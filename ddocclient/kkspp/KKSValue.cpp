...
void KKSValue::verify(void) const
{
    ...
    if(a_type == KKSAttrType::atBool){
        ...
        //соответственно, при записи в БД такие значения понимаются только в литеральном виде
        m_isLiteral = true;
        return;
    }
...
QDateTime KKSValue::stringToDateTime(const QString & s)
{
    ...
    //
    // потом пробуем взять только время из указанного формата
...
int KKSValue::setValue(const QString & _value, int _type)
{
            ...
            a_type == KKSAttrType::atCheckListEx)
    {
        ...
        //если данные пришли из БД, то разделителем будет запятая
        ...
        if(sl.count() < 2){
            ...
            //если данные пришли из БД, то значением what будет число и надо его преобразовать в строку
