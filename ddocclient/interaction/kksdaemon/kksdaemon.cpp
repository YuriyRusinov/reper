...
/* автоматическа€ разводка документов по рубрикам и выдача очередного периодического распор€жени€ 
(анализ журнала распор€жений на предмет наличи€ периодических)*/
...
void DDocServerListener::notify( char* notify_name, char * payload )
{
    ...
    QString sql = QString("select * from hGetHandlerInfo(%1)").arg(id); //также эта функци€ осуществл€ет изменение соответствующего пол€ handled_time
    KKSResult * res = m_db->execute(sql.toLocal8Bit().constData());
    if(!res)
        return;
    if(res->getRowCount() != 1){
