...
KKSFiltersEditorForm :: KKSFiltersEditorForm(const KKSCategory * _c, 
                                             const QString & tableName, //используетс€ только при обработке атрибутов типа atCheckListEx
 ...
 */
    ...
    else
    {
        ...
        else{//выбрана опци€ "поиск по всем атрибутам"
...
QString KKSFiltersEditorForm :: parseFilter(const KKSFilter * f, const QString & tableName)
{
    ...
    //ƒл€ случа€ если используетс€ оператор LIKE
    QString sLike;
    QString aVal; //сюда потом будем добавл€ть символ %, если необходимо
    if(cs == true)
        sLike = "LIKE";
    else
        sLike = "ILIKE";
