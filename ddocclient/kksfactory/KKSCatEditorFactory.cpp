 ...
 * Comment: данный класс €вл€етс€ фабрикой редакторов категорий
 ***********************************************************************/
...
void KKSCatEditorFactory :: addCategory (QWidget *ctw, int idCatType, bool isChild)
{
    ...
    {
        //
        // ¬ызов пришел от редактора категорий
        //
                                                           ...
                                                           Qt::WindowFlags f)
{
    ...
    else
    {
        ...
        cat->setType (ct);
        //if (cat && ct && (idCatType == 1 || idCatType == 2 || idCatType == 12))// категории данных типов €вл€ютс€ табличными
        KKSType * cTableT = loader->loadType (10);
        //indCat = new KKSCategory (-1, QString(), cTableT);
        //cat->setRecAttrCategory (indCat);
        //if (indCat)
        //    indCat->release ();
        if (cat && ct && ct->isQualifier())// категории данных типов €вл€ютс€ табличными
        {
            //
            // загрузка типа "подчиненна€ категори€"
            //
...
KKSCatEditor* KKSCatEditorFactory :: createCategoryEditor (KKSCategory *cat, // категори€
                                                           const KKSList<const KKSFilterGroup *> & filters,
                                                           bool isChildCat, // создаетс€ или редактируетс€ дочерн€€ категори€
...
void KKSCatEditorFactory :: findAttributes (int idAttrs, KKSAttributesEditor * attrsEditor)
{
    ...
    if (f->exec() == QDialog::Accepted)
    {
        ...
        //если используем фильтры дл€ справочника »ќ, то из него надо исключить системные справочники и журналы
