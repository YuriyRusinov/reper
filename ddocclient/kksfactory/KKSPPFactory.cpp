...
int KKSPPFactory::updateRubrics(KKSRubric * parent, int idMyDocsRubricator) const
{
    ...
    for(int i=0; i<cnt; i++){
        ...
        //по умолчанию считается что каждый экземпляр класса KKSRubric фвляется инициализированным
        //однако если мы работаем с деревом рубрик, загружая их через loadEIOList (в классе KKSRubricFactory)
        //и далее визуализируем через KKSRubricModel, то не вся информация о рубрике загружается из БД (только название и иконка)
        // реальная подгрузка данных о рубрике происходит только когда пользователь активизировал элемент дерева соответствующей рубрики
        //и именно в этот момент мы ставим рубрике setInitialized (true).
        //Фактически все это означает, что сохранять в БД мы будем лишь те рубрики, с которыми так или иначе работал пользователь.
        //необходимо отметить, что в неинициализированных рубриках вложения отсутствуют (также из БД не подгружены)
        //поэтому код, отвечающий за сохранение итемов для рубрик (см. ниже в этом методе) также выполняться не будет
        //parent->items().count() для него будет = 0
        if(r->isInitialized()){
            int ok;
            if(r->id() <= 0)
        ...
        int ok = updateRubrics(r, idMyDocsRubricator);
        if (ok < 0 )
            return ERROR_CODE;
    }
