...
KKSList<KKSAttrValue *> KKSLoader::loadAttrValues(KKSObject * io) const
{
    ...
    QString sql = QString("select * from ioGetObjectAttrs(%1)").arg(io->id());
    KKSResult * res = db->execute(sql);
    if(!res)
        return attrs;
...
QString KKSLoader::loadColumnValue(const KKSAttribute * a, 
                                   int id, 
                                   const QString & parentTable) const
{
    ...
    QString tableName = a->tableName().isEmpty() ? parentTable : a->tableName();
...
KKSMap<int, KKSCategoryAttr *> KKSLoader::loadCategoryAttrs(int idCategory) const
{
    ...
    for(int row=0; row<count; row++){
        ...
        if(!attr->columnName().isEmpty()){
            ...
            if (idRefType != KKSAttrType::atList)
            {
                                    ...
                                    const QString & parentTable) const
{
        ...
        )
    {
        ...
        QString cv = loadColumnValue(a, v.valueVariant().toInt(), parentTable);
        if(!cv.isEmpty())
            v.setColumnValue(cv);
        else
...
KKSCategory * KKSLoader::loadCategory(int id) const
{
...
KKSObject * KKSLoader::loadIO_id(int id) const
{
    return loadIO(id);
}

KKSObject * KKSLoader::loadIO(int id) const
{
    KKSObject * io = NULL;
...
KKSObject * KKSLoader::loadIO(const QString & tableName) const
{
    KKSObject * io = NULL;
...
KKSMap<int, KKSType *> KKSLoader::loadAvailableTypes (void) const
{
    ...
    KKSObject *typeRef = this->loadIO (IO_TYPE_ID);
    if (!typeRef)
        return typeList;

    KKSList<const KKSFilterGroup *> catTypeFilters;
    KKSList<const KKSFilter *> fCatTypes;
    const KKSFilter * fCatType = typeRef->category()->tableCategory()->createFilter (1, "select id from io_types where id >=1 and id<=7 or id=11 or id=12", KKSFilter::foInSQL);
    if (!fCatType)
        return typeList;
...
//        return sql;
    ...
    for (pca=tc->attributes().constBegin(); pca != tc->attributes().constEnd(); pca++)
    {
        ...
        if(a->type()->attrType() == KKSAttrType::atList ||
            ...
            KKSObject * refObj = this->loadIO (tName);
            if (!refObj)
                continue;
