...
В начале работы приложения необходимо получить указатель на экземпляр данного класса. За это отвечает класс KKSSito (методы KKSSito::getKKSSettings()).
...
Работа с настройками производится с помощью интерфейса класса библиотеки Qt QSettings. В данном классе доступны методы редактирования, чтения и восстановления параметров и поиска параметра по заданным имени группы и имени параметра.*/

class __CORE_EXPORT KKSSettings : public QSettings
{
    public:
