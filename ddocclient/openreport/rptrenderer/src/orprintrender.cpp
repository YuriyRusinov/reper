...
//    pPrinter->setPaperSize(QSizeF(pDocument->pageOptions().getCustomWidth(), pDocument->pageOptions().getCustomHeight()), QPrinter::A4);
  }
  else
    pPrinter->setPageSize((QPrinter::PageSize)psi.qpValue());
...
bool ORPrintRender::exportToHtml(ORODocument * pDocument, QString htmlFileName)
{
	int dpi=100;
  if(!pDocument)
    return false;
	...
	if(!htmlFile.open(QIODevice::WriteOnly))
		return false;
	QTextStream ht(&htmlFile);
	ht<<"<html><head><title>\n";
	ht<<pDocument->title()<<"\n</title>\n<body bgcolor='#ffffff'>";

    for(int page = 0; page < pDocument->pages(); page++)
    {
//<-----------------------------------------------------
...
//      QRectF rc = QRectF(0, 0, sz.width() * xDpi, sz.height() * yDpi);
//      painter->drawText(rc, tb->flags(), tb->text());
		ht<<"<div style=\"position:absolute; left: "<<ps.x()*dpi<<"; top: "<<ps.y()*dpi+960*page
			<<"; font-family: "<<tb->font().family()<<"; font-size: "<<tb->font().pointSize()<<"pt;\">"<<tb->text()<<"</div>\n";
    }
    else if(prim->type() == OROLine::Line)
    {
		OROLine * ln = (OROLine*)prim;
...
//		painter->setPen(pen);
//		painter->drawLine(QLineF(0, 0, (e.x()-s.x()) * xDpi, (e.y()-s.y()) * yDpi));
		if(abs(ln->endPoint().rx()-ln->startPoint().rx())>abs(ln->endPoint().y()-ln->startPoint().y()))
		ht<<"<table border='0' cellspacing ='0' cellpadding='0' style=\"position: absolute; top: "<<ln->startPoint().y()*dpi+960*page
			<<"; left: "<<ln->startPoint().rx()*dpi<<";\" bgcolor='"<<ln->brush().color().name()<<"'><tr height='"
			<<ln->weight()*dpi<<"'><td width='"
			<<abs(ln->endPoint().rx()-ln->startPoint().rx())*dpi<<"'></td><tr></table>\n";

		else
		ht<<"<table border='0' cellspacing ='0' cellpadding='0' style=\"position: absolute; top: "<<ln->startPoint().y()*dpi+960*page
			<<"; left: "<<ln->startPoint().rx()*dpi<<";\" bgcolor='"<<ln->brush().color().name()<<"'><tr height='"
			<<abs(ln->endPoint().ry()-ln->startPoint().ry())*dpi<<"'><td width='"
			<<ln->weight()*dpi<<"'></td><tr></table>\n";
    }
    else if(prim->type() == OROImage::Image)
    {
//      OROImage * im = (OROImage*)prim;
...
//	  painter->setPen(pen);
//      painter->drawRect(rc);
		ht<<"<table border='0' cellspacing ='0' cellpadding='0' style=\"position: absolute; top: "<<re->position().ry()*dpi+960*page
			<<"; left: "<<re->position().rx()*dpi<<";\" bgcolor='"<<re->brush().color().name()<<"'><tr height='"
			<<re->size().height()*dpi<<"'><td width='"
			<<re->size().width()*dpi<<"'></td><tr></table>\n";
    }
    else
    {
      qDebug("unrecognized primitive type");
