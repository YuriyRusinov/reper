...
void QueryEditor::mqlToggled(bool b)
{
  if(b && isVisible())
  {
    if(QMessageBox::question(this, tr("Change may be lost"), tr("Any changes you have made the the sql query may be lost by turning this option on. Are you sure you want to continue?"),
...
void QueryEditor::nameEditTextChanged(const QString & t)
{
  ...
  {
    XSqlQuery xqry;
    xqry.prepare("SELECT metasql_query FROM metasql WHERE metasql_group=:group and metasql_name=:name;");
