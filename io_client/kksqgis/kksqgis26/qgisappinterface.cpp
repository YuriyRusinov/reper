...
{
  //ksa qgis->styleSheetBuilder()->buildStyleSheet( opts );
}

void QgisAppInterface::saveStyleSheetOptions( const QMap<QString, QVariant>& opts )
{
  //ksa qgis->styleSheetBuilder()->saveToSettings( opts );
}
...
QFont QgisAppInterface::defaultStyleSheetFont()
{
  QFont f;
  return f;
  //ksa return qgis->styleSheetBuilder()->defaultFont();
}
...
{
  return false;//ksa QgsShortcutsManager::instance()->registerAction( action, defaultShortcut );
}

bool QgisAppInterface::unregisterMainWindowAction( QAction* action )
{
  return false;//ksa QgsShortcutsManager::instance()->unregisterAction( action );
}
