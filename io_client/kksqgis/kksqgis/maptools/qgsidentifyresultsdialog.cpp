...
//#include "qgisapp.h"
#include "KKSGISWidget.h" //!!!!! вместо qgisapp.h
...
QgsIdentifyResultsDialog::QgsIdentifyResultsDialog( KKSGISWidgetBase *w, QgsMapCanvas *canvas, QWidget *parent, Qt::WFlags f )
    : QDialog( parent, f )
    , mActionPopup( 0 )
    , mCanvas( canvas )
    , mDock( NULL )
    , mWorkingWidget(w)
{
  ...
  if ( myDockFlag )
  {
    mDock = new QgsIdentifyResultsDock( tr( "Identify Results" ) , mWorkingWidget );
    mDock->setAllowedAreas( Qt::LeftDockWidgetArea | Qt::RightDockWidgetArea );
    mDock->setWidget( this );
    /*ksa */ //mWorkingWidget->addWidget( Qt::LeftDockWidgetArea, mDock );
  }
...
void QgsIdentifyResultsDialog::contextMenuEvent( QContextMenuEvent* event )
{
  ...
  if ( featItem && vlayer && vlayer->actions()->size() > 0 )
  {
    ...
    for ( int i = 0; i < vlayer->actions()->size(); i++ )
    {
      ...
      QgsFeatureAction *a = new QgsFeatureAction( mWorkingWidget, action.name(), mFeatures[ featIdx ], vlayer, i, idx, this );
...
{
  connect( this, SIGNAL( activateLayer( QgsMapLayer * ) ), mWorkingWidget, SLOT( setActiveLayer( QgsMapLayer * ) ) );
  emit activateLayer( layer( lstResults->currentItem() ) );
  disconnect( this, SIGNAL( activateLayer( QgsMapLayer * ) ), mWorkingWidget, SLOT( setActiveLayer( QgsMapLayer * ) ) );
}
...
void QgsIdentifyResultsDialog::layerProperties( QTreeWidgetItem *item )
{
  ...
  mWorkingWidget->showLayerProperties( vlayer );
}
...
void QgsIdentifyResultsDialog::setWorkingWidget( KKSGISWidgetBase * w)
{
    mWorkingWidget = w;
}
