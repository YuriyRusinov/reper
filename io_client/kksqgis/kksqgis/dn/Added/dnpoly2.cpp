...
DNPoly2::DNPoly2(QString SerFileName)
{
...
QList<QPoint> DNPoly2::DefinePixels(int NumPixels,int x,int y)
{
 //QList<QPoint> DefPix;
...
void DNPoly2::GetSpectrPoint(int x,int y,float *DataSpec,bool *MaskCh)
{
...
void DNPoly2::GetDerivatSpectrPoint(int x,int y,int nCh1,int nCh2,float *LamMass,float *DerivatData)
{
 bool *MaskCh;
...
/*Взять спектр строки*/
void DNPoly2::GetSpectrString(int y,float *DataSpec,bool *MaskCh)
{
 QFile SerFile(this->SerFileName);
 qint64 Offset;
 int i,j,ThisCh,iCh;
...
void DNPoly2::GetSpectrZone(float *DataSpec,bool *MaskCh)
{
...
void DNPoly2::GetBrigthChanPoly(float *DataSpec,int nCh,bool *NoBlackPixels)
{
...
void DNPoly2::GetBrigthSqare(float *DataSpec,int nCh)
{
 bool *MaskChan;
 float *SpecStr;
 float proba;
...
void DNPoly2::MultiCh(float Kof)
{
 PolygonProp HeaderStruct;
 bool *MaskCh;
...
float DNPoly2::GetEntropPoint(int x,int y,int nPixels,quint64 KDiskr,int NumCh)
{
...
float DNPoly2::GetSKOPoint(int x,int y,int nPixels,int NumCh)
{
 QList <QPoint> SelectPix;
 bool *NoBlackPixels,*MaskCh;
 float SKOPix[1];
...
float DNPoly2::GetAKFPoint(int x,int y,int Side,float *Mask,int NumCh)
{
 bool *MaskCh;
 bool IsAreaFool;
...
quint64 DNPoly2::GetKolvoPointPoly(int x,int y,int *MassIsh,bool *NoBlackPixels)
{
 bool *Prover;
 QPoint Pix1,Pix2;
 int i,j_x,j_y;
...
void DNPoly2::GetMidlSpecPoly(bool *NoBlackPixels,unsigned long long KolvoPixPoly,float *MidlSpecPoly,bool *MaskCh)
{
 int jx,jy,i,ThisCh,iCh;
 float *SpecString;
 unsigned long long ip;
...
void DNPoly2::GetSKOPoly(bool *NoBlackPixels,unsigned long long KolvoPixPoly, float *SKOPoly, bool *MaskCh)
{
 int i,iCh;
 float *DataSpec;
...
void DNPoly2::GetEntropPoly(bool *NoBlackPixels,unsigned long long KolvoPixPoly, float *EntropPoly,quint64 KDistr,bool *MaskCh)
{
 int i,iCh;
 float *DataSpec;
 DataSpec=new float[KolvoPixPoly];
...
QList <DNPoly2::ClassToPoly> DNPoly2::ClassResultToPoly(float ProcentDetal)
{
 using namespace std;
 ...
 bool Obod[4];
 bool *PixProver;
 QList </*QList <QPoint>*/ClassToPoly> Polygons;
 /*QList <QPoint>*/ClassToPoly Polygon;
   ...
   //     fwrite(&Enter,sizeof(WORD),1,VektorFile);
     ...
     for(i=0;i<Number;i++)
     {
      if(KolvoKraiPoint[i]>0)
      {
       ...
       PolygonZap=TRUE;
       Polygon.Pix.clear();
    //Это надо для того, чтобы не все крайние точки подряд учитывались при построении полигона
       int UchPx=0;
       float PromRez=(ProcentDetal*KolvoKraiPoint[i])/100;
       UchPx=PromRez;
       int PixDelete;
       if(UchPx>0)
        PixDelete=KolvoKraiPoint[i]/UchPx;
       int PixConTrol=0;
       for(j=0;j<KolvoKraiPoint[i];j++)
       {
        ...
        if((UchPx>0 && (j==0 || j==KolvoKraiPoint[i]-1 || PixConTrol>=PixDelete))|| UchPx==0)
        {
         QPoint Pt;
         Pt.setX(KoordX);
         Pt.setY(KoordY);
...
void DNPoly2::PolygonExpGeo(QString FileName, double xTopLeft,double XD,double XAngle,double yTopLeft,double YD,double yAngle, QString NameClassif)
{
...
QList<QPoint> DNPoly2::PolygonImpGeo(QString FileName, double xTopLeft,double XD,double XAngle,double yTopLeft,double YD,double yAngle,bool **ClMass,QString *ClassName)
{
 QList <QPoint> pt;
// *ClMass=new bool[200];
// for(int i=0;i<200;i++)
...
QList<QPoint> DNPoly2::PolygonImpGeo(QString FileName, double xTopLeft,double XD,double XAngle,double yTopLeft,double YD,double yAngle)
{
 QList <QPoint> pt;
 QPoint ptOne;
 int KolvoPix,IntX,IntY;
...
quint64 DNPoly2::GetKolvoPointClass(int NumKl,int *MassIsh,bool *NoBlackPixels)
{
 quint64 KolPoint=0;
...
QList <int> DNPoly2::GetNumClassMass(int *MassIsh,bool *NoBlackPixels)
{
...
void DNPoly2::GetMidleSpectrClass(float *DataSpec,quint64 KPoint,int NumClass,int *MassIsh,bool *NoBlackPixels,bool *MaskCh)
{
 float *SpecString;
 //quint64 KPoint;
 // Узнаём количество каналов интереса
...
void DNPoly2::AddChanal(QString Formula)
{
 PolygonProp HeaderStruct;
 bool *MaskCh;
...
void DNPoly2::AddChanal(float *Brigth)
{
 PolygonProp HeaderStruct;
 bool *MaskCh;
 float *SpecString,*SpecStringNew;
...
void DNPoly2::AddIntegralChanal(int nCh1,int nCh2)
{
 PolygonProp HeaderStruct;
 bool *MaskCh;
 float *SpecString,*SpecStringNew;
...
void DNPoly2::AddIntegralChanal(int nCh1,int nCh2,float *LamMass)
{
 PolygonProp HeaderStruct;
 bool *MaskCh;
 float *SpecString,*SpecStringNew;
...
void DNPoly2::CreateIndexImg(QString BMPFileName,int NumCh)
{
    FILEHEADER bfhFile;
    INFOHEADER bihFile;
    RGBColor *OutBuf;
...
void DNPoly2::AddEntropChanal(int nPixels,quint64 KDiskr,int NumCh)
{
 bool *MaskCh;
 float *SpecString,*SpecStringNew;
 float *SpecPoint;
...
void DNPoly2::AddSKOChanal(int nPixels, int NumCh)
{
 bool *MaskCh;
 float *SpecString,*SpecStringNew;
 float *SpecPoint;
...
void DNPoly2::AddAKFChanal(int Side, int NumCh,int xb,int yb)
{
 bool *MaskCh;
 float *SpecString,*SpecStringNew;
 float *SpecPoint;
...
float DNPoly2::CalcMidleBrigth(int NumCh1,int NumCh2,float *SpecData)
{
...
/*Линейная фильтрация*/
/*Для канала*/
float* DNPoly2::LinearFilter(int side,float *Apert,float Kof,int NumCh)
{
 float *NewCh;
 float *Brith;
...
void DNPoly2::LinearFilter(int side,int *Apert,float Kof)
{
 int *NewClassMass;
...
/*Волшебная палочка*/
quint64 DNPoly2::MagicWand(bool *NoBlackPixels,int x,int y,float Contrast,int NumCh, int mode,bool *NoBlackPixels2)
{
 x=x-this->xn;
 y=y-this->yn;
 quint64 KolvoPx;
...
/*IsoData*/
int DNPoly2::IsoData(bool *NoBlackPixels,int KolvoClass,float MaxSKO,float Qc,int I, bool *MaskCh)
{
 DNMathAdd ThisMath;
 quint64 KPix;
 int ThisCh,KolvoI;
...
int DNPoly2::SAM(bool *NoBlackPixels,QStringList PolyFileNames,bool IsSKOEnabled,float LimitSKO,bool *MaskCh)
{
 DNPoly2 *ThisPoly;
...
// QList <float> ModVector;
 ...
 for(int i=0;i<PolyFileNames.size();i++)
 {
  ...
  ThisPoly=new DNPoly2(PolyFileNames[i]);
...
/*Метод MaxLike*/
void DNPoly2::MaxLike(bool *NoBlackPixels,QStringList PolyFileNames,bool *MaskCh)
{
 DNPoly2 *ThisPoly;
 int KolvoCh;
 quint64 KolPoint;
...
/*Фильтр*/
void DNPoly2::FilterPix(bool *NoBlackPixels,QString Formula,int Usl, float Value2)
{
 QString DataStr;
 DNCalcString Cls;
 float *Res,Value;
...
void DNPoly2::FilterPoly(int *MassIsh,QString Formula,int Usl, float Value2)
{
 bool *ProverPix;
 bool *NoBlackPixels;
 bool *MaskCh;
...
void DNPoly2::Batinometr(int N590,int N830,int N900,
                        int N580,int N620,
...
void DNPoly2::MaskAreaMS(float DeltaMS,int TypeFone,int Nnir1,int Nnir2,int Nred,int Ngr,float IndexPor,float LRM, float AreaPor)
{
...
void DNPoly2::MaskAreaGS(QString ClsId,QList<QString> ClsBackgroId,
...
void DNPoly2::RoutMoveTap1(int Ngreen,int Nred,int Nnir,int NredEdge,float NDVIpor,int NumCl,float MaxSKO,float Qc,int I)
{
 bool *NoBlackPix;
 NoBlackPix=new bool[this->W*this->H];
...
void DNPoly2::RoutMoveTap2(int N630,int N690,int N750,int N860,int N720,int N800,int N900,int N970,
...
void DNPoly2::SmocesTap1(int Nred,int Nnir,float NDVIPor)
{
 QString Formula;
 Formula="(["+QString().setNum(Nnir+1)+"]-["+QString().setNum(Nred+1)+"])/"+
         "(["+QString().setNum(Nnir+1)+"]+["+QString().setNum(Nred+1)+"])";
...
void DNPoly2::SmocesTap2(float R600Por,float KofCorrPor,int N600,int N500)
{
 bool *NoBlackPix;
 bool *ProvPix;
...
void DNPoly2::EmbedObjMS(int Nred,int Nnir,float NDVIPor,float DNDVI,int Nnir2,float B630_690,float B800_1100,float Cpor630_690,float Cpor800_1100)
{
 QString Formula;
 Formula="(["+QString().setNum(Nnir+1)+"]-["+QString().setNum(Nred+1)+"])/"+
         "(["+QString().setNum(Nnir+1)+"]+["+QString().setNum(Nred+1)+"])";
...
//Векторизация
void DNPoly2::PixToGeo(int xp,int yp,double xTopLeft,double XD,double XAngle,double yTopLeft,double YD,double yAngle,double *xGeo,double *yGeo)
{
...
void DNPoly2::GeoToPix(double *xp,double *yp,double xTopLeft,double XD,double XAngle,double yTopLeft,double YD,double yAngle,double xGeo,double yGeo)
{
 double /*A,B,C,D,*/ypp;
// A=XD*cos(XAngle);
// B=YD*sin(XAngle);
...
QList <DNVector> DNPoly2::RastrToVector(double xTopLeft,double XD,double XAngle,double yTopLeft,double YD,double YAngle,float Kof,float MinV, QString FileName,
                                        QList <QString> NameFields,QList <QString> TypeFields,QString NameLayer)
{
 ...
 DNVector Polygon;
 DNVector::ClassLayer Properties;
...
//     fwrite(&Enter,sizeof(WORD),1,VektorFile);
  ...
  for(i=0;i<Number;i++)
  {
   if(KolvoKraiPoint[i]>0)
   {
    ...
    Polygon.GPt.clear();
    Polygon.TypeLayer=0;
    Polygon.NameLayer=NameLayer;
    Polygon.EPSG=FileName;
    Polygon.Vol=j_pol*Kof+MinV;

    for(int den=0;den<NameFields.size();den++)
    {
     Properties.NameField=NameFields[den];
     Properties.TypeField=TypeFields[den];
     Properties.VolPolygon=QString().setNum(Polygon.Vol,'d',4);
     Polygon.Polygons<<Properties;
    }
...
bool* DNPoly2::FillMaskCh(QList <int> nCh)
{
