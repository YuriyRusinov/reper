...
QgsAttributeTableDialog::QgsAttributeTableDialog( KKSGISWidgetBase * w, QgsVectorLayer *theLayer, QWidget *parent, Qt::WindowFlags flags )
    ...
    , mWorkingWidget(w)
{
  ...
  mWorkingWidget = qobject_cast<KKSGISWidgetBase *>(parent);
  ...
  mMainView->init( mLayer, mWorkingWidget->mapCanvas(), QgsFeatureRequest(), context );
  ...
  connect( this, SIGNAL( saveEdits( QgsMapLayer * ) ), mWorkingWidget, SLOT( saveEdits( QgsMapLayer * ) ) );

  bool myDockFlag = settings.value( "/qgis/dockAttributeTable", false ).toBool();
  if ( myDockFlag )
  {
    mDock = new QgsAttributeTableDock( tr( "Attribute table - %1 (%n Feature(s))", "feature count", mMainView->featureCount() ).arg( mLayer->name() ), mWorkingWidget );
    mDock->setAllowedAreas( Qt::BottomDockWidgetArea | Qt::TopDockWidgetArea );
    mDock->setWidget( this );
    connect( this, SIGNAL( destroyed() ), mDock, SLOT( close() ) );
    //ksa mWorkingWidget->addDockWidget( Qt::BottomDockWidgetArea, mDock );
  }
...
{
  mWorkingWidget->saveEdits( mLayer, true, true );
}
...
void QgsAttributeTableDialog::on_mAddFeature_clicked()
{
  ...
  QgsFeature f;
  QgsFeatureAction action( mWorkingWidget, tr( "Geometryless feature added" ), f, mLayer, -1, -1, this );
  //action.setWorkingWidget(mWorkingWidget);
...
void QgsAttributeTableDialog::on_mToggleEditingButton_toggled()
{
  if ( !mLayer )
    return;
  if ( !mWorkingWidget->toggleEditing( mLayer ) )
  {
...
void QgsAttributeTableDialog::on_mRemoveAttribute_clicked()
{
  ...
  if ( dialog.exec() == QDialog::Accepted )
  {
    ...
    {
      mWorkingWidget->messageBar()->pushMessage( tr( "Attribute error" ), tr( "The attribute(s) could not be deleted" ), QgsMessageBar::WARNING, mWorkingWidget->messageTimeout() );
      mLayer->destroyEditCommand();
    }
    // update model - a field has been added or updated
    masterModel->reload( masterModel->index( 0, 0 ), masterModel->index( masterModel->rowCount() - 1, masterModel->columnCount() - 1 ) );
...
void QgsAttributeTableDialog::setFilterExpression( QString filterString )
{
  ...
  {
    mWorkingWidget->messageBar()->pushMessage( tr( "Parsing error" ), filterExpression.parserErrorString(), QgsMessageBar::WARNING, mWorkingWidget->messageTimeout() );
    return;
  }
  ...
  if ( filterExpression.hasEvalError() )
  {
    mWorkingWidget->messageBar()->pushMessage( tr( "Error filtering" ), filterExpression.evalErrorString(), QgsMessageBar::WARNING, mWorkingWidget->messageTimeout() );
    return;
  }
}

void QgsAttributeTableDialog::setWorkingWidget( KKSGISWidgetBase * w)
{
    mWorkingWidget = w;
}
