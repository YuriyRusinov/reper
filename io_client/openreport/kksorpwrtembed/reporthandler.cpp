...
void ReportHandler::docToolBars(QMainWindow * mw, int /*edge*/, bool /*newLine*/)
{
  ...
  {
    // create the toolbars
    tbFiles = mw->addToolBar(tr("File Operations"));
    ...
    if(!_noDatabase)
    {
      tbDatabase = mw->addToolBar(tr("Database Operations"));
    ...
    tbEdit = mw->addToolBar(tr("Edit Operations"));
    ...
    tbOptions = mw->addToolBar(tr("Layout Options"));
    ...
    tbItems = mw->addToolBar(tr("Report Elements"));
...
QAction * ReportHandler::populateMenuBar(QMenu * menu, QAction * exitAction)
{
  ...
  if(!_noDatabase)
  {
    QMenu * mDb = menu->addMenu(tr("Data&base"));
    connect(mDb, SIGNAL(aboutToShow()), this, SLOT(dbMenuAboutToShow()));
    mDb->addAction(dbLoadAction);
    mDb->addAction(dbSaveAction);
    //if(_allowDBConnect)
  ...
  QMenu * mInsert = menu->addMenu(tr("&Insert"));
  mInsert->addAction(itemLabelAction);
  mInsert->addAction(itemFieldAction);
  mInsert->addAction(itemTextAction);
  mInsert->addAction(itemLineAction);
  ...
  QMenu * mDoc = menu->addMenu(tr("&Document"));
  mDoc->addAction(docTitleAction);
  mDoc->addAction(docPageSetupAction);
  mDoc->addAction(docQuerySourceListAction);
  mDoc->addAction(docSectionEditorAction);
...
void ReportHandler::dbConnect(const QString & dbName, 
                              const QString & ip,
                              const QString & port,
                              const QString & user,
                              const QString & passwd)
{
    ...
    if(dbConnected) {
        ...
        // connect
          ...
          OpenRPT::databaseURL = QString("QPSQL://") + ip + QString(":") + port + QString("/") + dbName;

          QSqlDatabase db = databaseFromURL(OpenRPT::databaseURL);
          if (!db.isValid()){
              QMessageBox::warning( _parentWindow, tr("No Database Driver"),
          ...
          bool result = db.open();
          if (!result){
              QMessageBox::critical( _parentWindow, tr("Cannot Connect to Database Server"),
          ...
          XSqlQuery pkglist("SELECT 1 as a;");
          while(pkglist.next())
          {
              QString a = pkglist.value("a").toString();
...
void ReportHandler::dbLoadDoc(qint64 idReport) 
{
    QSqlDatabase db = QSqlDatabase::database(QSqlDatabase::defaultConnection, FALSE);
    if(!db.isValid()) {
        QMessageBox::warning(0, tr("No Database Connection"),
    ...
    if(doc.setContent(getSource(idReport), &errMsg, &errLine, &errCol)){
        DocumentWindow * rw = fileOpen(doc);
        if(rw){
            // setup any variables to let this doc know how it was loaded
            // so when a save command is issued it knows what to do
            rw->_scene->lastSaveToDb = TRUE;
            rw->_scene->dbRecordName = getNameById(idReport);
            rw->_scene->dbRecordGrade = getGradeById(idReport);
        }
    } 
    else {
        QMessageBox::warning(0, tr("Error Loading Report"),
...
QString ReportHandler::getSource(qint64 idReport)
{
  if(idReport > 0) {
    XSqlQuery src_qry;
    src_qry.prepare(getSqlFromTag("fmt13", QSqlDatabase::database().driverName()));	// MANU
    src_qry.bindValue(":report_id", idReport);
    src_qry.exec();
    if(src_qry.first())
...
QString ReportHandler::getNameById(qint64 idReport)
{
  if(idReport > 0)
  {
	XSqlQuery src_qry;
...
int ReportHandler::getGradeById(qint64 idReport)
{
  if(idReport > 0)
  {
	XSqlQuery src_qry;
...
DocumentWindow * ReportHandler::activeDocumentWindow()
{
    ...
    {
        QMdiArea * area = qobject_cast<QMdiArea*>(_parentWindow);
        if(!area){
            ...
            QWidget * w = area->activeSubWindow() ? area->activeSubWindow()->widget() : 0;
            if(w->inherits("DocumentWindow"))
                return qobject_cast<DocumentWindow*>(w);
            return 0;
        }
    }
...
void ReportHandler::print(qint64 idReport, bool showPreview)
{
    QSqlDatabase db = QSqlDatabase::database(QSqlDatabase::defaultConnection, FALSE);
    if(!db.isValid()) {
        QMessageBox::warning(0, tr("No Database Connection"),
    ...
    if(!ddoc.setContent(getSource(idReport), &errMsg, &errLine, &errCol)){
        QMessageBox::warning(0, tr("Error Loading Report"),
                    QString().sprintf(tr("ReportWriterWindow::dbLoadDoc() : ERROR on setContent()\n\t%s (Line %d Column %d)").toLatin1().constData(),errMsg.toLatin1().data(),errLine,errCol));
        return;
    }
    ...
    QMdiArea * area = qobject_cast<QMdiArea*>(_parentWindow);
    if(area)
        parent = area->activeSubWindow() ? area->activeSubWindow()->widget() : 0;
...
void ReportHandler::onSubWinChanged(QMdiSubWindow* w)
{
  bool show = w ? true : false;
  dbSaveAction->setEnabled(show & dbLoadAction->isEnabled());
  grpEdit->setEnabled(show);
  ...
  //tbFiles->setVisible(show);
  //tbDatabase->setVisible(show);
  tbEdit->setVisible(show);
  tbOptions->setVisible(show);
  tbItems->setVisible(show);
  tbFont->setVisible(show);
