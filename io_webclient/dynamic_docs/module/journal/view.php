...
if($pattern != '' && $pattern_flag == true && !empty($pattern))
        {
        	$query = 'SELECT count(DISTINCT ID) as num from jGetCmdsEx(date(NOW() - 7), date(NOW()), false);';
        }
        else 
        {

			$query = 'SELECT count(DISTINCT ID) as num from jGetCmdsEx(date(NOW()), date(NOW()), false);';
        }
	...
	if($pattern != '' && $pattern_flag == true)
        {
        	...
        	$query = 'SELECT DISTINCT * from jGetCmdsEx(date(NOW()), date(NOW()), false);';
        }
        else 
        {
        	$query = 'SELECT DISTINCT * from jGetCmdsEx(date(NOW() - 7), date(NOW()), false);';
        }
