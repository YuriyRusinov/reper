...
if($pattern != '')
        {
        	$query = 'SELECT COUNT(*) AS num FROM iogetfiles('.$id.') WHERE date(insert_time) \''.db_class::escape_like($pattern).'\';';
        }
        else 
        {

			$query = 'SELECT COUNT(*) AS num  FROM iogetfiles('.$id.');';
        }
	...
	if($pattern != '')
        {
        	...
        	$query = 'SELECT * FROM iogetfiles('.$id.') WHERE date(insert_time) = \''.db_class::escape_like($pattern).'\';';
        }
        else 
        {
        	$query = 'SELECT * FROM iogetfiles('.$id.') ;';
        }
...
include SITE_FILE_ROOT.'template/'.$STORAGE['module'].'/files.php';
