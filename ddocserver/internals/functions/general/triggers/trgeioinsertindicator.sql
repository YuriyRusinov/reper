create or replace function eioInsertIndCheck() returns trigger as
$BODY$
declare
    r record;
    rr record;
  ...
  /*
    if(TG_OP = 'INSERT') then
        ...
        loop
            ...
            tableName := 'eio_object_' || r.table_name || '_ref_' || r.id;
...
select f_safe_drop_trigger('trgeioinsertindicator', 'rec_attrs_values');

select f_create_trigger('trgeioinsertindicator', 'before', 'insert or update', 'rec_attrs_values', 'eioinsertindcheck()');
