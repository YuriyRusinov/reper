...
begin
    ...
    if(TG_OP = 'UPDATE') then
        ...
        if(hasRName is not null and hasRName = TRUE) then
            ...
            if(new.rr_name is null or new.rr_name = 'New record' or new.rr_name = 'Ќова€ запись') then
    ...
    tableName = TG_RELNAME;--«десь замену делвть не надо, поскольку sequence переименовываетс€ всегда и называетс€, начина€ с tbl_
        ...
        if(new.unique_id is not null) then
            ...
            if(hasRName is not null and hasRName = TRUE) then
                if(new.rr_name isnull) then
                    new.rr_name = 'new record'; --при информационном обмене системные параметры записи будут обновлены позднее создани€ записи в таблице. 
                                                --ѕоэтому мы зададим произвольное значение полю rr_name. ј оно потом будет автоматически обновлено до правильного
                end if;
            end if;
...
--    end if;
    ...
    --Ёто позвол€ет всегда иметь код атрибута (т.е. в т.ч. название колонки в таблицах справочников)
