...
create or replace function getIncidentCard(int8) returns setof h_get_message_card as 
$BODY$
declare
    id_v alias for $1;
...
begin
    ...
    for r in select * from getIncidentCard(uuid_v)
    loop
        return next r;
    end loop;
...
create or replace function getIncidentCard(varchar) returns setof h_get_message_card as 
$BODY$
declare
    uuid_v alias for $1;
    r h_get_message_card%rowtype;
begin
    ...
    for r in select * from getIncidentCard(uuid_v::uuid)
    loop
        return next r;
    end loop;
...
create or replace function getIncidentCard(uuid) returns setof h_get_message_card as
$BODY$
declare
...
begin
    ...
    for r in 
        ...
        select q.uuid_t, q.uuid_t::varchar --'MONITORING' (от аргуса)
