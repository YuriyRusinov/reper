CREATE OR REPLACE FUNCTION createpassport(integer, integer, integer[], integer)
  RETURNS xml AS
$BODY$
declare
...
begin
    ...
    if(regNumber is not null) then
        ...
        select unique_id into regNum from io_objects where id = idObject;
    ...
    if(msgKind = 2 and docName is not null) then
        xml_str := xml_str || docName;
    end if;
    ...
    xml_str := xml_str || E'\t\t\t\t\t<doc_name><![CDATA[';
    if(msgKind = 1 and docName is not null) then
        xml_str := xml_str || docName;
    end if;
    xml_str := xml_str || E']]></doc_name>\n';
    ...
    select count(*) into cnt from urls_objects uo where uo.id_io_object=idObject;
    xml_str := xml_str || E'\t\t\t\t<attached_files_set with_files_amnt="' || cnt || E'">\n';
    if (cnt > 0) then
        i := 1;
        for rec_url in
            select uo.id_url, uo.name from urls_objects uo where uo.id_io_object=idObject
        loop
