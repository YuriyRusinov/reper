--spesial version of acl_secureTable() with alter sequence
create or replace function acl_secureTable_shu(varchar) returns int4 as
$BODY$
declare
...
begin
    ...
    if(substring(tableName from 1 for 4) <> 'shu_') then
        raise exception 'You cannot execute special acl_secureTable() function on non SHU_ tables!';
        return -1;
    end if;
