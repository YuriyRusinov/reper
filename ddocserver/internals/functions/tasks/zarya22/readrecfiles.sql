create or replace function readRecFiles (int4) returns varchar as
$BODY$
declare
    idRec alias for $1;
    ...
    fileSize int8;
begin
    select count(*) into cnt from urls_records where id_record=idRec;
    ...
    if (cnt is not null and cnt > 0) then
        ...
        i := 1;
        for r in 
            select id_url from urls_records ur where ur.id_record=idRec
        loop
