...
--Данная функция используется при информационном обмене
--не вызывается aFindEqual для поиска подходящего атриута
-- осуществляется поиск атрибута только с таким же уникальным идентификатором
...
begin
    ...
    select id into idAttr from attributes where unique_id = uniqueID;
    if(idAttr is not null and idAttr > 0) then
        return idAttr;
    end if;
...
create or replace function aInsert1(int4,
                                     varchar,
                                     varchar,
                                     varchar,
                                     varchar,
