create or replace function recAddToRubric (int8, int4, VARCHAR) returns int4 as
$BODY$
declare
...
begin
    ...
    if (cnt is not null and cnt > 0) then
        update rubric_records set r_icon = rIcon where id_record=idRecord and id_rubric=idRubric;
        return 1;
    end if;
