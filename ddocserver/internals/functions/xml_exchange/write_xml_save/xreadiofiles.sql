create or replace function xReadIOFiles (int4) returns varchar as
$BODY$
declare
...
begin
    ...
    if (cnt is not null and cnt > 0) then
        str_res :=  E'\t\t<attached_files count="' || cnt || E'">\n';
        i := 1;
        
        for r in 
            select id_url from urls_objects uo where uo.id_io_object=idObject
        loop
            str_res := str_res || E'\t\t\t<file number="' || i || E'">\n';
            ...
            str_res := str_res || E'\t\t\t</file>\n';
            i := i+1;
        end loop;
    else
        str_res :=  E'\t\t<attached_files count="0">\n';
    end if;

    str_res := str_res || E'\t\t</attached_files>\n';
